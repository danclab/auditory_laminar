...
surfaces={'pial','white'};
subj_surf_dir=fullfile(params.subjects_dir, subj_id, 'surf');
...
for s_idx=1:length(surfaces)
    ...
    ds=gifti(ds_fname);
    norm=compute_surface_normals(ds_fname, orig_fname, 'link_vector');
    ds.normals=norm;
    ds_lv_fname=fullfile(subj_surf_dir, sprintf('%s.ds.link_vector.gii', surface));
    save(ds,ds_lv_fname);
    ds_final=remove_deep_vertices(fullfile(params.subjects_dir, subj_id), ds, orig, ds_lv_fname, orig_fname);
